...
-- Run this in Supabase SQL Editor - handles existing tables
...
DROP VIEW IF EXISTS products_with_ratings CASCADE;
...
CREATE POLICY "Admins can view all profiles" ON profiles FOR SELECT USING (is_admin = true);
CREATE POLICY "Users can CRUD addresses" ON addresses FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can CRUD wishlists" ON wishlists FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can CRUD cart" ON cart_items FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users view own orders" ON orders FOR SELECT USING (auth.uid() = user_id OR is_admin = true);
CREATE POLICY "View approved reviews" ON reviews FOR SELECT USING (is_approved = true);
